...
def main():
    ...
    def preprocess_function(examples):
        ...
        for inp, tgt in zip(inputs, targets):
            ...
            full_input_ids = input_tokens["input_ids"] + target_tokens["input_ids"]
            full_attention_mask = (
                input_tokens["attention_mask"] + [1] * len(target_tokens["input_ids"])
            )

            # Mask out input part from labels
            labels = (
                [-100] * len(input_tokens["input_ids"]) + target_tokens["input_ids"]
            )
    ...
    if training_args.do_predict:
        ...
        input_model = 'xx'
        # test files
        langs = ['eng', 'zho', 'nav', 'saa', 'arp', 'kuk']
        for lang in langs:
            file_path = "/home/common/ACNLP/m-AMR2Text/" + lang + "_sent_test_smelt.txt"  
            all_generations = []
            ...
            try:
                ...
                for input_line in input_lines:
                    ...
                    test_dataset = Dataset.from_dict(example)
                    ...
                    if trainer.is_world_process_zero():
                        if training_args.predict_with_generate:
                            ...
                            all_generations.append({
                                'input': input_line,
                                'generation': test_preds[0]
                            })

                            
                            # Print current generation
                            print(f"\nInput: {input_line}")
                            print(f"Generation: {test_preds[0]}")
                            # print("-" * 50)
                            print("---------")
                            print(line_n)
                            print("---------")
                ...
                # output
                output_file = "/home/common/ACNLP/gemma-llama-finetune/pred_results/gemma/eng_sent/pred" + lang + ".text.txt"
